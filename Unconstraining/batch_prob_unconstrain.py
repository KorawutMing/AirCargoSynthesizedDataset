...
SAVE_DIR = "Unconstraining/results/probabilistic_unconstrained_3yr"
LOOKBACK = 365
...
def process_od_pair(args):
    ...
    # 3.5 Probabilistic Expected Max Demands (Z = 1.645 for 95% confidence)
    Z = 1.645
    exp_max_kg = {}
    exp_max_cbm = {}
    
    for seg in SEGMENTS.keys():
        mean_kg = sample_df[f"Observed_{seg}_kg"].shift(1).rolling(window=LOOKBACK, min_periods=1).mean().fillna(0)
        std_kg = sample_df[f"Observed_{seg}_kg"].shift(1).rolling(window=LOOKBACK, min_periods=1).std().fillna(0)
        exp_max_kg[seg] = mean_kg + Z * std_kg
        
        mean_cbm = sample_df[f"Observed_{seg}_cbm"].shift(1).rolling(window=LOOKBACK, min_periods=1).mean().fillna(0)
        std_cbm = sample_df[f"Observed_{seg}_cbm"].shift(1).rolling(window=LOOKBACK, min_periods=1).std().fillna(0)
        exp_max_cbm[seg] = mean_cbm + Z * std_cbm

    total_exp_max_kg = pd.DataFrame(exp_max_kg).sum(axis=1).replace(0, np.nan)
    total_exp_max_cbm = pd.DataFrame(exp_max_cbm).sum(axis=1).replace(0, np.nan)
    ...
    for seg in SEGMENTS.keys():
        ...
        # Calculate dynamic Probabilistic shares
        share_kg = (exp_max_kg[seg] / total_exp_max_kg).fillna(1.0 / len(SEGMENTS)).values
        share_cbm = (exp_max_cbm[seg] / total_exp_max_cbm).fillna(1.0 / len(SEGMENTS)).values
